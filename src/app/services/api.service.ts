...
export class ApiService {
  ...
  StoreCategory = (CategoryData) => {
    return this.http.post(`${environment.baseUrl}/category/store`, CategoryData, {
  ...
  DeleteMedia = (id) => {
    return this.http.delete(`${environment.baseUrl}/product/media/deleteProductMedia/${id}`).toPromise();
  };
  /* Stocks */
  GetStocks = (category?) => {
    return this.http.get(`${environment.baseUrl}/stock${category ? `/?category_id=${category}` : ''}`).toPromise();
  };
  GetStockById = (id) => {
  ...
  GetTopStocks = () => {
    return this.http.get(`${environment.baseUrl}/topStocks`).toPromise();
  }
  GetTrendingStocks = () => {
    return this.http.get(`${environment.baseUrl}/trendingStocks`).toPromise();
  }
}
