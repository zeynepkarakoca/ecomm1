...
export class ProductsComponent implements OnInit {
  ...
  handleDeleteProductImage = (imageId) => {
    this.dialog.confirm({
      nzTitle: "Delete photo",
      nzContent: 'Are you sure you want to delete this photo ?',
      nzOkText: 'Yes',
      nzOkType: 'danger',
      nzOnOk: () => {
        this.api.DeleteMedia(imageId).then((res: any) => {
          this.message.success(res?.message);
          this.getProducts();
        })
          .catch(error => this.message.error(error && error.error.message));
      }
    });
  };
